...
def sample_input_features(key, batch_size, num_features):
    ...
    return sample_normal_matrix(key, shape=(batch_size, num_features))
...
def assign_class_labels(inputs, num_classes):
    ...
    selected=inputs[:,:num_classes]
    ans=jnp.argmax(selected,axis=-1)
    ans=ans.astype(jnp.int32)
    return ans
...
def one_hot_encode_labels(labels, num_classes):
    ...
    arr=labels[:, None] == jnp.arange(num_classes)[None, :]
...
def classification_accuracy(logits, labels):
    ...
    comparison=(pred == labels)# булев массив [True, False, ...]
    return jnp.mean(comparison)   # скаляр
...
def train_mlp(params, x, one_hot_targets, learning_rate, num_epochs):
    ...
    for i in range(num_epochs):
        params, loss=training_step(params, x, one_hot_targets, learning_rate) # возвращает кортеж из двух вещей
    return params
